...
int main()
{
	...
	strcpy_s(tStudent.strName, "가나다라 adsf");
	

	// strcat_s 함수는 문자열을 붙여주는 기능이다.
	// 오른쪽에 있는 문자열을 왼쪽에 붙여준다 strName 에는 위에ㅓㅅ
	// 가나다라 asdf 를 저장해 두었기 때문에 이 뒤에 오른쪽 문자열을 붙여서
	// 만들어준다.
	...
	// 문자열의 끝은 항상 0(NULL)로 끝나야 한다. 그런데 쓰레기 값이 들어가있는
	// 상태에서 아래처럼 각 배열 요소에 값을 넣어주게 되면 그 값이 출력되고
	// 넣어주지 않은 부분은 쓰레기 값으로 그대로 출력된다. 왜냐하면 출력할 때
